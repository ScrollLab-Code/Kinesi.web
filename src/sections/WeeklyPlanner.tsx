...
export default function WeeklyPlanner() {
  ...
  const getSubjectColor = (sub: string) => {
    ...
    if (s.includes("anat")) return "border-rose-400 bg-rose-50/40 text-rose-800"
    if (s.includes("fisio")) return "border-teal-400 bg-teal-50/40 text-teal-800"
    if (s.includes("histo")) return "border-amber-400 bg-amber-50/40 text-amber-800"
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="glass-card rounded-2xl p-5 border border-amber-500/20 clinical-shadow space-y-4">
          ...
          <form onSubmit={handleAddBlock} className="space-y-3.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Materia / Tema</label>
                <input
                  type="text"
                  value={subject}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Detalle o Actividad</label>
              <input
                type="text"
                value={topic}
                onChange={e => setTopic(e.target.value)}
                placeholder="ej. Plexo braquial y ramas"
                required
                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs outline-none focus:border-amber-500"
              />
            </div>
        ...
        <div className="space-y-4">
          {/* Navegación por Días */}
          <div className="flex gap-1.5 overflow-x-auto scrollbar-none pb-1 border-b border-slate-100">
            {daysOfWeek.map(d => {
